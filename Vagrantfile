...
Vagrant.configure("2") do |config|
  ...
  config.vm.define "dev-parsoid" do |c|
    c.vm.box = "ubuntu/trusty64"
    c.vm.provision "shell", path: "./parsoid/provision.sh", args: "#{DEV_PROTOCOL} #{DEV_HOST}"
  ...
  config.vm.define "prod-parsoid" do |c|
    c.vm.box = "awsdummy"
    c.vm.provision "shell", path: "./parsoid/provision.sh", args: "#{PROD_PROTOCOL} #{PROD_HOST}"
    c.vm.provider :aws do |aws, override|
      aws.aws_dir = "./.aws/"
      aws.keypair_name = "fw"
      aws.ami = "ami-09dc1267"
      aws.instance_type = "t2.micro"
      aws.security_groups = ["parsoid"]
      aws.tags = {
        'Name' => PROD_PARSOID_TAG
      }
      aws.block_device_mapping = [{ 'DeviceName' => '/dev/sda1', 'Ebs.VolumeSize' => 16 }]
      override.ssh.username = "ubuntu"
      override.ssh.private_key_path = "fw.pem"
