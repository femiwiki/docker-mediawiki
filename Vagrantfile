...
Vagrant.configure("2") do |config|
  config.vm.define "dev-www" do |c|
    ...
    c.vm.provision "shell", path: "./www/provision.sh", args: "#{DEV_PROTOCOL} #{DEV_HOST} #{DEV_DB} #{ENV['DB_PW']} #{DEV_PARSOID}"
  ...
  config.vm.define "prod-www" do |c|
    c.vm.box = "awsdummy"
    c.vm.provision "shell", path: "./www/provision.sh", args: "#{PROD_PROTOCOL} #{PROD_HOST} #{PROD_DB} #{ENV['DB_PW']} #{PROD_PARSOID}"
